...
export const ListOportunidadeFetch = async() => {
    ...
    let result = await iService.fetch({
        ...
        properties: `Id CargaHoraria Status Cidade Salario DataLimite Experiencia NumVagas Detalhes IsFinalizado TipoFuncao { Designacao } TipoFormacao { Designacao } TipoFuncaoId Provincia { Designacao } Nacionalidade { Designacao } Estado { Designacao } createdAt updatedAt`,
    });
    let { data } = result;
    data = data.map((d) => {
