...
var IService = (function() {
    ...
    IService.prototype.join = async function(schema, table, isMutation) {
        let response = await fetch(this.uri, { method: "post", body: JSON.stringify(schema), headers: { "Content-Type": "application/json" } })
    ...
    IService.prototype.onCreateSchema = function({ value, getById, type, table, properties }) {
        this.table = table;
        let query = null;
        switch (type) {
            ...
            case servicetypes.STORE:
                let _Itable = table.charAt(0).toLowerCase() + table.substr(1, table.length - 1).toLowerCase();
                let Itable = table.charAt(0).toLocaleUpperCase() + table.substr(1, table.length - 1).toLowerCase();
