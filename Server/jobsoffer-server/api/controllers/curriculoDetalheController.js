...
exports.insert = (req, res, next) => {
   let data = new Date(Date.now())
        // assign values
    const curriculoDetalheId = uuid()
    const sysdate = new Date(Date.now())
  
    CurriculoDetalhe.create({
                Id: curriculoDetalheId,
...
req.body.UpdatedAt=new Date().toJSON()
    CurriculoDetalhe.update(req.body, { where: { Id: req.params.id } }).then(() => {
...
exports.delete = (req, res, next) => {
    ...
    CurriculoDetalhe.destroy(req.params.id).then(() => {
