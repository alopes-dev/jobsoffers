...
exports.insert = (req, res, next) => {
   let data = new Date(Date.now())
        // assign values
    const curriculoHobesId = uuid()
    const sysdate = new Date(Date.now())
  
    CurriculoHobes.create({
                Id: curriculoHobesId,
...
req.body.UpdatedAt=new Date().toJSON()
    CurriculoHobes.update(req.body, { where: { Id: req.params.id } }).then(() => {
...
exports.delete = (req, res, next) => {
    ...
    CurriculoHobes.destroy(req.params.id).then(() => {
