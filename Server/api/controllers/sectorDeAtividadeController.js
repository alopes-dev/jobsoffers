...
exports.insert = (req, res, next) => {
   let data = new Date(Date.now())
        // assign values
    const sectorDeAtividadeId = uuid()
    const sysdate = new Date(Date.now())
  
    SectorDeAtividade.create({
                Id: sectorDeAtividadeId,
...
req.body.UpdatedAt=new Date().toJSON()
    SectorDeAtividade.update(req.body, { where: { Id: req.params.id } }).then(() => {
...
exports.delete = (req, res, next) => {
    ...
    SectorDeAtividade.destroy(req.params.id).then(() => {
